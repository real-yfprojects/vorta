...
class ArchiveTab(ArchiveTabBase, ArchiveTabUI, BackupProfileMixin):
    ...
    def mount_action(self):
        ...
        dialog = choose_file_dialog(
            self, self.tr("Choose Mount Point"), want_folder=True
        )
        dialog.open(receive)
    ...
    def umount_result(self, result):
        ...
        if result["returncode"] == 0:
            ...
            self._set_status(
                self.tr("Unmounting failed. Make sure no programs are using {}").format(
    ...
    def list_archive_result(self, result):
        ...
        if result["returncode"] == 0:
            ...
            def process_result():
                def receive():
                    ...
                    if extraction_folder:
                        ...
                        if params["ok"]:
                            self._toggle_all_buttons(False)
                            job = BorgExtractJob(
                                params["cmd"], params, self.profile().repo.id
                            )
                            job.updated.connect(self.mountErrors.setText)
                ...
                dialog = choose_file_dialog(
                    self, self.tr("Choose Extraction Point"), want_folder=True
                )
                dialog.open(receive)
    ...
    def delete_action(self):
        ...
        if self.archive_name is not None:
            if not self.confirm_dialog(
                ...
                trans_late(
                    "ArchiveTab", "Are you sure you want to delete the archive?"
                ),
            ):
                return
    ...
    def delete_result(self, result):
        if result["returncode"] == 0:
            ...
            deleted_row = self.archiveTable.findItems(
                self.archive_name, QtCore.Qt.MatchExactly
            )[0].row()
        ...
        def process_result():
            ...
            if archive_cell_older and archive_cell_newer:
                ...
                params = BorgDiffJob.prepare(
                    profile, archive_name_older, archive_name_newer
                )
