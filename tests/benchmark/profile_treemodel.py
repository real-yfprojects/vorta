...
def walk_model(model: QAbstractItemModel):
    indices = [QModelIndex()]
...
def main(repo, a1, a2):
    ...
    try:
        ...
        model = diff_result.DiffTree()
        diff_result.parse_diff_json(json_lines, model)

        model.setMode(model.DisplayMode.TREE)
        cProfile.runctx('walk_model(model)', globals(), locals(),
                        'profile_stats-tree')
        ...
        proxymodel = diff_result.DiffSortProxyModel()
        proxymodel.setSourceModel(model)

        model.setMode(model.DisplayMode.TREE)
        cProfile.runctx('walk_model(proxymodel)', globals(), locals(),
                        'profile_stats-sortedtree')
        model.setMode(model.DisplayMode.SIMPLIFIED_TREE)
        cProfile.runctx('walk_model(proxymodel)', globals(), locals(),
                        'profile_stats-sortedsimp')
        model.setMode(model.DisplayMode.FLAT)
        cProfile.runctx('walk_model(proxymodel)', globals(), locals(),
                        'profile_stats-sortedflat')
