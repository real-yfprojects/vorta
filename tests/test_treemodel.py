...
class TestFileSystemItem:
    def test_tuple(self):
        item = FileSystemItem(PurePath('test'), 0,
                              [FileSystemItem(PurePath('test/hello'), 2)])

        assert item[0] == item.path
        assert item[1] == item.data
        assert item[2] == item.children
    ...
    def test_add(self):
        ...
        child = FileSystemItem(PurePath('test/hello'), 4)
        ...
        with pytest.raises(RuntimeError):
            # may not add a child twice
            item.add(child)

        child = FileSystemItem(PurePath('test/hello'), 8)
        with pytest.raises(RuntimeError):
    ...
    def test_remove(self):
        item = FileSystemItem(PurePath('test'), 0)
        child1 = FileSystemItem(PurePath('test/a'), 4)
        child2 = FileSystemItem(PurePath('test/b'), 3)
        child3 = FileSystemItem(PurePath('test/c'), 2)
    ...
    def test_get(self):
        ...
        assert res[0] == 0 and res[1] == child1

        res = item.get('b')
        assert res is not None
        assert res[0] == 1 and res[1] == child2
    ...
    def test_get_subpath(self):
        ...
        child11 = FileSystemItem(PurePath('test/a/aa'), 4)
        child1.add(child11)

        assert item.get_subpath(PurePath('a/aa')) is child11
        assert item.get_subpath(PurePath('b')) is child2
...
class TestFileTreeModel:
    def test_basic_setup(self):
        ...
        model.addItem((PurePath('test'), 0))
        assert model.rowCount() == 1

        item = model.getItem(PurePath('test'))
        ...
        model.addItem(FileSystemItem(PurePath('/hello/test'), 2))

        assert model.rowCount() == 2

        item = model.getItem(PurePath('/'))
        assert item is not None
        assert item.subpath == '/'
        assert len(item.children) == 1

        item = model.getItem(PurePath('/hello/test'))
        assert item is not None and item.data == 2

        # test adding Item to existing parent
        model.addItem((PurePath('test/subtest'), 3))
        assert model.rowCount() == 2
        item = model.getItem(PurePath('test/subtest'))
        ...
        # test index
        item1 = model.getItem(PurePath('test'))
        ...
        # test remove
        model.removeItem(PurePath('test'))
        assert model.rowCount() == 1
        assert item1 not in model.root.children

        model.removeItem(PurePath('/hello/test'))
        assert model.rowCount() == 1
